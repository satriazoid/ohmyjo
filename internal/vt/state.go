...
const (
	...
	attrWide
	// attrWideTail marks the second cell, which carries no glyph of its own.
	attrWideTail
)
...
const (
	...
	ModeMouseX10
	ModeMouseMany
	// ModeBracketedPaste is DECSET 2004: pasted text must be wrapped in
	// ESC[200~ / ESC[201~ so a bracketed application can tell it from typing.
	ModeBracketedPaste
...
const (
	...
	ChangedTitle
	// ChangedScrollback is set when lines are added to or removed from
	// scrollback, so a scrollbar can update without polling.
	ChangedScrollback
)
...
// line is an alias so screen rows and history rows are the same type.
type line = Line
...
type State struct {
	...
	// history holds lines scrolled off the top of the screen, and sbOffset
	// is how far the viewport is scrolled back into it. See scrollback.go.
	history     *history
	sbOffset    int
	cursorShape uint8
}
...
func newState(w io.Writer) *State {
	return &State{
		...
		history:       newHistory(DefaultScrollback),
	}
}
...
// Lock locks the state so a caller can read a consistent snapshot.
func (t *State) Lock() {
	t.mu.Lock()
}

// Unlock releases the lock. Unlike upstream vt10x this does not double as a
// damage reset: conflating the two means an unmatched Unlock panics, and it
// forces a renderer to reset damage at a moment it does not control. Call
// ClearDamage explicitly once the changed rows have been painted.
func (t *State) Unlock() {
	t.mu.Unlock()
}

// ClearDamage resets per-line damage and the change mask. The caller must hold
// the lock, normally via Lock.
func (t *State) ClearDamage() {
	t.resetChanges()
}
...
// clearWideAt blanks the other half of any wide glyph occupying column x, so
// overwriting one cell never leaves a stale half-glyph behind. A wide glyph
// that is half overwritten would otherwise render two cells of garbage and
// desynchronise the rest of the row.
func (t *State) clearWideAt(x, y int) {
	if y < 0 || y >= t.rows || x < 0 || x >= t.cols {
		return
	}
	// The cell to the left may be a head whose tail sits at x.
	if x > 0 && t.lines[y][x-1].Mode&attrWide != 0 {
		t.lines[y][x-1].Mode &^= attrWide
		t.lines[y][x-1].Char = ' '
	}
	// The cell here may be a head whose tail sits at x+1.
	if t.lines[y][x].Mode&attrWide != 0 && x+1 < t.cols {
		t.lines[y][x+1].Mode &^= attrWideTail
		t.lines[y][x+1].Char = ' '
	}
	t.dirty[y] = true
}

// setWideChar writes a two-cell glyph: the head at x carries the rune, and the
// continuation cell at x+1 is marked as its tail so a renderer skips it.
func (t *State) setWideChar(c rune, attr *Glyph, x, y int) {
	t.clearWideAt(x, y)
	if x+1 < t.cols {
		t.clearWideAt(x+1, y)
	}
	t.setChar(c, attr, x, y)
	t.lines[y][x].Mode |= attrWide
	if x+1 < t.cols {
		t.setChar(' ', attr, x+1, y)
		t.lines[y][x+1].Mode |= attrWideTail
	}
}

// setChar stores one cell. It does not resolve wide-glyph overlap; callers go
// through clearWideAt or setWideChar for that.
...
func (t *State) setCursorStyle(n int) {
	if n < 0 || n > CursorBarBlink {
		return
	}
	t.cursorShape = uint8(n)
	t.changed |= ChangedScreen
...
// clipWide drops the halves of wide glyphs that a shrink has cut in two. A head
// without its tail, or a tail without its head, would render as a stray cell
// and shift the visible text of the row.
func (t *State) clipWide() {
	for y := 0; y < t.rows; y++ {
		row := t.lines[y]
		for x := 0; x < len(row); x++ {
			if row[x].Mode&attrWide == 0 {
				continue
			}
			if x+1 >= len(row) {
				row[x].Mode &^= attrWide
		...
		if n := len(row); n > 0 && row[n-1].Mode&attrWideTail != 0 && (n < 2 || row[n-2].Mode&attrWide == 0) {
			row[n-1].Mode &^= attrWideTail
			row[n-1].Char = ' '
			t.dirty[y] = true
		}
	}
...
func (t *State) resize(cols, rows int) bool {
	...
	// A shrink can cut a wide glyph in half; neither half is valid on its own.
	t.clipWide()
	return slide > 0
}
...
func (t *State) scrollUp(orig, n int) {
	...
	// Lines leaving the top of the full screen are the ones that belong in
	// scrollback; a scroll region that does not start at row 0 is a
	// full-screen-application construct and must not pollute history.
	push := orig == 0 && t.bottom == t.rows-1 && t.mode&ModeAltScreen == 0
...
func (t *State) insertBlanks(n int) {
	...
	// The inserted blank cells sit between dst-1 and dst; either side of that
	// boundary may now hold half of a wide glyph.
	t.clearWideAt(dst-1, t.cur.Y)
	t.clearWideAt(dst, t.cur.Y)
}
...
func (t *State) deleteChars(n int) {
	...
	// The shifted text may have started on half of a wide glyph.
	t.clearWideAt(dst, t.cur.Y)
	t.clearWideAt(src-1, t.cur.Y)
}
