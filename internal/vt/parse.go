...
func (t *State) parse(c rune) {
	...
	// A combining mark or a zero-width control rune must not advance the
	// cursor: the grid is addressed in cells, and treating one as a cell would
	// shift every following glyph by one column.
	w := RuneWidth(c)
	if w == 0 {
		return
	}

	if w == 2 && t.cur.X+w > t.cols {
		// A wide glyph needs two adjacent cells. At the right edge it moves to
		// the next line, exactly as if the row had filled up.
	...
	if w == 2 {
		t.setWideChar(c, &t.cur.Attr, t.cur.X, t.cur.Y)
	} else {
